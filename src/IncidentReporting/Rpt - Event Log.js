function test() {
  generateEventLogReport("1PRrS-rglvdbPM9g0gY5oZvS0-dGD29Nm");
}
  ...
  try {
    ...
    if (sheetLastRow == 1) {
      ...
      if (incidentStartDate === incidentEndDate) {
        incidentDates = Utilities.formatDate(new Date(incidentStartDate), tz, "MMMM dd, yyyy");
      } else {
        incidentDates = Utilities.formatDate(new Date(incidentStartDate), tz, "MMMM dd, yyyy") + " - " + Utilities.formatDate(new Date(incidentEndDate), tz, "MMMM dd, yyyy");
      }
      Logger.log(incidentDates);
